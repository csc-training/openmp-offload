...
int main(void) {
  int runningOnGPU = 0;
#ifdef _OPENMP
#pragma omp target device(1) map(tofrom:runningOnGPU)
  {
...
if (runningOnGPU)
{
#pragma omp target teams num_teams(5) device(1) map(from:runningOnGPU)
  {
    printf("Hello World!  \n The default device is %d. There are %d devices available. Device %d is used.\n", omp_get_default_device(),  omp_get_num_devices(), omp_get_device_num());
